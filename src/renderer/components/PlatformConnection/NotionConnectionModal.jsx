...
const NotionConnectionModal = ({ isOpen, onClose }) => {
  ...
  const connection = usePlatformStore((state) =>
    state.connections.find((c) => c.platform === 'notion')
  );

  const platformId = 'notion';
  ...
  useEffect(() => {
    ...
    const initialData = { integrationToken: '' };
  ...
  const debouncedSave = useCallback(
    async (platform, credentials, fieldName) => {
      ...
      try {
        ...
        if (response.success) {
          showSuccess('Integration Token saved successfully');
          setHasChanges(true);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const localToken = formDataRef.current.integrationToken?.trim();
      if (localToken) {
        await saveConnection(platformId, { integrationToken: localToken });
      }
      if (!localToken && !connection?.hasCredentials) {
        showError('Integration Token is required.');
        setIsLoading(false);
        return;
      }
      ...
      if (response.success) {
        ...
        showError(response.error || 'Failed to connect Notion');
      }
    } catch (error) {
      showError(error.message || 'Failed to connect Notion');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (response.success) {
        setHasChanges(true);
        showSuccess('Notion disconnected successfully.');
        setFormData({ integrationToken: '' });
        formDataRef.current = { integrationToken: '' };
        const currentConnections = usePlatformStore.getState().connections;
        const index = currentConnections.findIndex((c) => c.platform === platformId);
        if (index >= 0) {
          const newConnections = [...currentConnections];
  ...
  const hasToken = () => !!(connection?.hasCredentials || formData.integrationToken?.trim());
  const showPassword = showPasswords['integrationToken'] || false;
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 pb-4 border-b border-border-muted">
            ...
            <div>
              ...
              <p className="text-sm text-text-secondary">
                {isFullyConnected ? 'Notion is connected' : 'Paste your Integration Token to connect'}
              </p>
            </div>
          </div>
          ...
          {isFullyConnected && (
            <div className="p-4 bg-success/10 border border-success/20 rounded-lg">
              <p className="text-sm text-success font-medium">✓ Notion is connected and ready to use</p>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-text-secondary text-sm font-medium" htmlFor="integrationToken">
                Integration Token <span className="text-error ml-1">*</span>
              </label>
              {(connection?.credentials?.hasIntegrationToken || connection?.hasCredentials) && !savingField && (
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => handleInputChange('integrationToken', e.target.value)}
              ...
              <button
                type="button"
                onClick={() => setShowPasswords((prev) => ({ ...prev, integrationToken: !showPassword }))}
            ...
            <p className="mt-2 text-xs text-text-muted">
              ...
              <span
                ...
                onClick={() => window.systemAPI?.openExternal('https://www.notion.so/my-integrations')}
              >
                notion.so/my-integrations
              </span>
              {' → Create integration → Copy Internal Integration Token (starts with secret_)'}
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-border-muted">
            {isConnected ? (
              <>
                <ButtonPlain variant="outline" className="w-full border-error text-error hover:bg-error/10" onClick={handleDisconnect} isLoading={isLoading} disabled={isSaving}>
                  Disconnect
                </ButtonPlain>
                <p className="mt-2 text-xs text-text-secondary text-center">Disconnect to remove saved token and connection status</p>
              </>
            ) : (
              <>
                <ButtonPlain variant="primary" className="w-full" onClick={handleConnect} isLoading={isLoading} disabled={isSaving || !hasToken()}>
                  Connect
                </ButtonPlain>
                <p className="mt-2 text-xs text-text-secondary text-center">No login required — just paste your Integration Token and connect</p>
              </>
            )}
          </div>
...
export default NotionConnectionModal;
